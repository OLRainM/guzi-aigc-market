...
| 5 | 发布前端到端与失败场景验收 | 执行注册登录、越权、伪造/超限上传、商品发布、订单闭环、AI 失败恢复、页面刷新和错误降级。 | **进行中**；已建立可切换 `E2E_BASE_URL` 的 Playwright 基线，覆盖匿名保护、注册登录刷新和无效订单参数；前端已补全局错误边界、可取消查询 Hook 和统一加载状态，完整商品/订单/AI 闭环仍需可控测试数据与部署环境。 | `P1`；依赖事项 1–4 和可用部署环境；是内测前门槛。 |
