...
- **项目**：谷子交易与 3D/AIGC 平台
- **记录日期**：2026-09-05
- **当前阶段**：核心 MVP 已完成并发布为 `v1.0.0`；阶段 7 已完成远端部署、API/Worker/Web 重启恢复验收、基础接口验收和受控只读压测。备份恢复演练、完整安全/依赖检查、真实 Provider 联调和内测闭环仍未完成。
...
- **E2E 基线**：`apps/web` 已加入 Playwright 配置、`e2e`/`e2e:report` 脚本和 4 个用例；默认目标为本机 `http://localhost:5173`，远端验收需显式设置 `E2E_BASE_URL`。当前已验证配置加载、用例枚举和 Web 生产构建；注册登录刷新、订单异常用例需在可控 API/数据库环境执行，禁止默认写入公网环境。
...
| 顺序 | 事项名称 | 简要说明 | 当前状态 | 优先级 / 依赖 |
| --- | --- | --- | --- | --- |
| 4 | 补齐 API 单元测试与关键集成测试 | 覆盖认证、越权、上传、商品状态、订单幂等/库存、AI 全生命周期、重复回调和管理员权限。 | **已完成当前计划范围**；核心包、管理员能力、跨用户订单权限和关键异常路径已有测试，统一发布检查入口已接入 API 全量测试。 | `P1`；后续仅随新功能补回归测试。 |
| 5 | 发布前端到端与失败场景验收 | 执行注册登录、越权、伪造/超限上传、商品发布、订单闭环、AI 失败恢复、页面刷新和错误降级。 | **进行中**；已建立可切换 `E2E_BASE_URL` 的 Playwright 基线，覆盖匿名保护、注册登录刷新和无效订单参数；前端已补全局错误边界、可取消查询 Hook、统一加载组件、商品详情/管理后台查询迁移和生成轮询卸载保护，完整商品/订单/AI 闭环仍需可控测试数据与部署环境。 | `P1`；依赖事项 1–4 和可用部署环境；是内测前门槛。 |
...
| 7 | 备份策略与恢复演练 | 制定 MySQL、对象存储和必要配置的备份保留、校验和恢复步骤，至少完成一次恢复演练。 | **未完成**；已有部署前备份目录和记录，无自动备份策略及恢复验收。新增发布清单已明确备份权限、校验和及隔离恢复要求。 | `P1`；依赖事项 6；恢复演练不得破坏生产数据。 |
| 8 | 结构化日志、Request ID 与基础监控 | 统一 API/Worker 日志字段，串联请求和任务，补充关键指标、容器健康、队列积压与错误告警。 | **部分完成**；API 已有 `X-Request-ID` 和健康检查，本轮补充了发布检查入口，但结构化访问日志、Worker 链路和指标告警仍不完整。 | `P1`；依赖事项 5、6；先覆盖核心链路。 |
| 9 | 生产安全与依赖检查 | 检查公网端口、默认凭据、Cookie/CORS、敏感配置、依赖漏洞、生产构建产物及上传/鉴权边界。 | **部分完成**；公网隔离、JWT 和基础安全头已验收；Web 镜像改用锁文件和 `npm ci`，前端依赖声明已固定到锁定版本，新增统一发布检查入口及清单。`govulncheck`、`pip-audit`、`npm audit`、Trivy 的完整结果仍待工具和网络可用时取得。 | `P1`；依赖部署环境和事项 4–8；公开内测前完成。 |
...
| 12 | 发布问题清单与 MVP 修复闭环 | 汇总测试/内测问题，按严重度分级，修复发布阻断项并重新验收，形成版本基线。 | **部分完成**；`v1.0.0` 已部署公网并完成基础接口验收、重启恢复验收和 300 请求/10 并发只读压测；当前无本轮压测错误，但真实 Provider、内测和完整安全审计相关问题仍待外部条件满足后关闭。 | `P1`；依赖事项 5–11；作为阶段 7 收尾。 |
