...
- **下一步**：补齐管理后台与跨模块测试，随后进行浏览器 E2E、恢复、安全和运维验收。
...
3. 最小管理后台与审计日志（已完成）。
4. 补齐 API 管理权限/跨模块测试，再进行发布前测试、恢复、安全和运维验收。
...
1. 事项 1–3 已完成，作为当前 MVP 行为基线。
2. 完成事项 4，补齐管理权限、跨模块异常和契约测试。
3. 通过事项 5–9 建立 E2E、恢复、备份、安全和可观测性基线。
