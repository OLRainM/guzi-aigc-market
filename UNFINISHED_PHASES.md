...
- **项目**：谷子交易与 3D/AIGC 平台
- **记录日期**：2026-09-05
- **当前阶段**：阶段 1–5 与阶段 6 的认证、商品/资产、3D 查看器、收藏/个人中心、交易沙盒、模拟订单、AI 生成 HTTP 闭环、一键发布和生成可靠性均已完成；阶段 7 的发布前验证与运维尚未完成。
- **当前基线**：核心 MVP 已可部署并在 `http://8.154.28.98` 验收；Mock Provider 可用，真实 AI Provider 仍需人工配置密钥并联调。
- **权威未完成清单**：见本文「十一、未完成事项与推荐开发顺序」；该节按依赖和优先级排序，后文阶段 6/7 仅作范围对照。
...
- **开始时间**：2026-09-05
- **当前进度状态**：已完成整理，待按顺序实施
- **已完成进展**：核心业务 P0、模拟订单、一键发布和生成可靠性已完成并有测试覆盖；未完成事项已按依赖关系归档。
- **当前阻塞事项**：真实 Provider 密钥、正式支付资质/账号和内测参与者需要人工提供。
- **下一步**：优先完成商品/订单一致性小缺口，再实现用户确认优化结果和最小管理后台，随后进行发布前验证与运维验收。
...
下面按落地顺序记录**已完成**能力。每步写清：改了什么、关键函数、调用约定、验收、以及本步明确不做的事。未完成项仍在后文阶段 6 / 7 的 `[ ]` 中。
...
本步**不**做 OAuth、邮箱验证、管理员后台。RBAC 只预置 `USER`/`ADMIN` 种子，管理功能见阶段 6 未完成项。
...
新增 `apps/api/internal/catalog`，GORM `AutoMigrate(&Product{}, &ProductAsset{})`。状态只允许 `DRAFT → PUBLISHED → OFF_SHELF`，禁止第三处改状态。
...
8. `offShelf`：`PUBLISHED → OFF_SHELF`
...
- `getContent`：未发布且非卖家是 **403**（详情是 404）
...
- 无 `OFF_SHELF → PUBLISHED` 重新上架
- `getContent` 与 `get` 对未发布商品的对外错误码不一致（403 vs 404）
...
1. `placeOrder`：`Idempotency-Key` 必须是 UUID；同买家同 key 同 hash 返回原单（HTTP 仍 201），hash 不同则 `IDEMPOTENCY_CONFLICT`
...
- `POST /orders` 必须带 `Idempotency-Key`；成功一律 201
...
已勾选的实现步骤、方法与代码位置见上文「一、当前已完成事项」。此处只保留对照清单；未勾选的才是后续要做的。
...
历史上已经完成的生成正确性、一键发布和生成可靠性不再列入待办。当前所有未完成事项及其唯一推荐顺序见「十一、未完成事项与推荐开发顺序」。

近期执行主线：

1. 商品与订单行为一致性修正。
2. 用户确认或修改优化后的 Prompt。
3. 最小管理后台与审计日志。
4. 发布前测试、恢复、安全和运维验收。
...
本节是当前唯一的未完成事项清单。排序原则是：先修正已知行为不一致，再补齐核心 MVP 的前置能力；核心链路稳定后完成发布验收与运维；最后再接入真实支付、审核和生产化扩展。`P0` 为核心闭环或发布阻断项，`P1` 为影响可靠性、验收或小范围内测的事项，`P2` 为试运营和生产化扩展。
...
| 2 | 用户确认或修改优化后的 Prompt | 创建生成任务前展示原始 Prompt、RAG 摘要、结构化参数和优化 Prompt，允许编辑并确认；服务端保存确认版本、最终 Prompt 和版权确认。 | **未开始**；当前 Worker 在任务执行阶段优化，前端只有生成后的结果展示。 | `P0`；依赖现有 `PromptOptimizer` 输出和生成任务模型；先稳定预览契约，再改创建接口和工作台。 |
| 3 | 最小管理后台与审计日志 | 管理员查看用户/商品、下架商品、查看及重试失败 AI 任务；记录操作者、目标、动作、前后状态、请求 ID 和时间。 | **未开始**；已有 `ADMIN` 角色和鉴权基础，尚无管理路由、页面、审计模型。 | `P0`；依赖 RBAC、商品状态机和生成重试；应在内测前完成。 |
...
- OpenAPI 尚未完整覆盖认证、账户、收藏、订单和管理接口，且部分描述仍标为“预留”：在事项 1–4 修改接口时同步补齐契约，不单独延后。
...
1. 完成事项 1，统一商品、资产和订单行为基线。
2. 完成事项 2，补齐 AI 主流程的用户确认环节。
3. 完成事项 3，补齐最小运营闭环和审计。
4. 通过事项 4–9 建立测试、恢复、安全和可观测性基线。
5. 按演示目标决定事项 10；无真实 Provider 账号时保持 Mock 并明确标识。
6. 完成事项 11–12，执行内测并关闭发布阻断问题，作为 MVP 收尾。
7. MVP 验收后按业务数据推进事项 13–20，不提前建设高成本扩展。
